...
function box(x,y,z,w,h,d,mat=0x6f7b86){
  const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),new THREE.MeshStandardMaterial({color:mat,roughness:.78}));
...
box(0,9,-45,90,20,2,0x59636d);
box(0,9,45,90,20,2,0x59636d);
box(-45,9,0,2,20,90,0x59636d);
box(45,9,0,2,20,90,0x59636d);
...
player.position.set(0,2.2,25);
scene.add(player);
const torso=box(0,-.65,0,1.0,1.4,.55,0x2f5f91);
torso.position.set(0,-.65,0);
player.add(torso);
torso.visible=false;
...
const crouchSpeed=4.5;
const slideStartSpeed=17;
const gravity=24;
const jump=9.5;
const slideFriction=11;
let locked=false;
let sliding=false;
let slideVelocity=new THREE.Vector3();
const standingCameraY=1.6;
const crouchCameraY=.9;
...
function animate(){
  ...
  if(locked){
    ...
    // SHIFT + movement + CTRL starts a slide. Releasing CTRL does not cancel it.
    ...
    if(sliding){
      ...
      if(newSpeed===0){
        ...
        slideVelocity.setLength(newSpeed);
      }
    ...
    for(const [x,y,z,w,dz] of plats){
      if(Math.abs(px-x)<w/2&&Math.abs(pz-z)<dz/2&&player.position.y>=y&&player.position.y<=y+2.5){
        ground=y+1.05;
      }
    }
    if(player.position.y<=ground){
      ...
      vy=0;
      if(keys.Space&&!crouching&&!sliding)vy=jump;
    }
